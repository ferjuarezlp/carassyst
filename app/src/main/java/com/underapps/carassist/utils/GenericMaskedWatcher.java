...
public class GenericMaskedWatcher implements TextWatcher {

    private String mMask;
    private String mResult = "";

    public GenericMaskedWatcher(String mask) {
        mMask = mask;
    }
    ...
    public void afterTextChanged(Editable s) {
        ...
        try { /* prepare the formatter*/
            MaskedFormatter formatter = new MaskedFormatter(mask);
            formatter.setValueContainsLiteralCharacters(false);
            formatter.setPlaceholderCharacter((char) 1); /* get a string with applied mask and placeholder chars*/
            value = formatter.valueToString(value);
            try { /* find first placeholder*/
